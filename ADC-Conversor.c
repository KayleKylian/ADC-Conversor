...
void update_leds_pwm() {
    ...
    uint brightness_blue = map_value(adc_value_y, 0, 4095, LED_WRAP, 0);  // LED Azul (inverte eixo)
    uint brightness_red = map_value(adc_value_x, 0, 4095, LED_WRAP, 0);   // LED Vermelho (inverte eixo)
...
void display_border(ssd1306_t *ssd, uint8_t typeBorder) {
    if (typeBorder == 0) {
        ...
        for (uint8_t x = 0; x < WIDTH; x += 6) {
            ...
            ssd1306_hline(ssd, x, x + 4, HEIGHT - 1, true);  // Linha inferior
        }
        ...
        for (uint8_t x = 0; x < WIDTH - 4; x += 4) {
            ...
            ssd1306_pixel(ssd, x, HEIGHT - 1, true); // Linha inferior corrigida
        }
